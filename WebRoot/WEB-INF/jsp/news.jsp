...
<script src="<%=path%>/statics/js/jquery.min.js"></script>
    ...
    <link href="<%=path%>/statics/css/list.css" type="text/css" rel="stylesheet">
</head>
        ...
        <div class="place column" name="header_nav">
            <div class="module" style="height:20px;">
                <ul class="containerss">
                   ...
                   <!--  <li>
                       <a href="evergreen-magazine.jsp" title="" style="text-decoration: none">《长青》杂志</a>
                       <div>
    ...
    <div class="row pd-t40">
        <div class="col-xs-12 col-sm-2 over-hide bg-fff pd-b50">
            <ul class="left_menu pd-t5">
               <li class="active">
                      <c:if test="${details.detailsTypeId==1}">   <a href="pilotage"></c:if>
                      <c:if test="${details.detailsTypeId==2}"> <a href="about-news"></c:if>
                      <c:if test="${details.detailsTypeId==3}"> <a href="course-arrangement"></c:if>
                      <c:if test="${details.detailsTypeId==4}"> <a href="past-course"></c:if>
                      <c:if test="${details.detailsTypeId==5}"> <a href="specialty-instruction"></c:if>
                      <c:if test="${details.detailsTypeId==6}"> <a href="teacher-presence"></c:if>
                      <c:if test="${details.detailsTypeId==7}"> <a href="school-world"></c:if>
                      <c:if test="${details.detailsTypeId==8}"><a href="street-dynamic"></c:if>
                       <c:if test="${details.detailsTypeId==9}"><a href="evergreen-magazine"></c:if>
                        <c:if test="${details.detailsTypeId==10}"><a href="classic-review"></c:if>
                         <c:if test="${details.detailsTypeId==11}"><a href="activity-silhouette"></c:if>
                      <c:if test="${details.detailsTypeId==12}"><a href="school-world?detailsTypeId=12"></c:if>
                      <c:if test="${details.detailsTypeId==13}"><a href="school-world?detailsTypeId=13"></c:if>
                      <c:if test="${details.detailsTypeId==14}"><a href="school-world?detailsTypeId=14"></c:if>
                      <c:if test="${details.detailsTypeId==15}"><a href="school-world?detailsTypeId=15"></c:if>
                      <c:if test="${details.detailsTypeId==16}"><a href="school-world?detailsTypeId=16"></c:if>
                      <c:if test="${details.detailsTypeId==17}"><a href="school-newspaper"></c:if>
                      <c:if test="${details.detailsTypeId==0}">单位概况</c:if>
                        <c:if test="${details.detailsTypeId==1}">党建领航</c:if>
                          <c:if test="${details.detailsTypeId==2}">新闻动态</c:if>
                         <c:if test="${details.detailsTypeId==3}">课程安排</c:if>
                         <c:if test="${details.detailsTypeId==4}">往期课程</c:if>
                         <c:if test="${details.detailsTypeId==5}">专业介绍</c:if>
                         <c:if test="${details.detailsTypeId==6}">教师风采</c:if>
                         <c:if test="${details.detailsTypeId==7}">学员天地</c:if> 
                         <c:if test="${details.detailsTypeId==8}">镇街动态</c:if>
                         <c:if test="${details.detailsTypeId==9}">长青杂志</c:if>
                         <c:if test="${details.detailsTypeId==10}">经典回顾</c:if>
                         <c:if test="${details.detailsTypeId==11}">活动剪影</c:if>
                         <c:if test="${details.detailsTypeId==12}">书法作品</c:if>
                         <c:if test="${details.detailsTypeId==13}">绘画作品</c:if>
                         <c:if test="${details.detailsTypeId==14}">瓷绘作品</c:if>
                         <c:if test="${details.detailsTypeId==15}">摄影作品</c:if>
                         <c:if test="${details.detailsTypeId==16}">文学作品</c:if>
                         <c:if test="${details.detailsTypeId==17}">校报在线</c:if>
                    </a>
                </li>
               <!--  <li>
                   <a href="#">通知通告</a>
        ...
        <div class="col-xs-12 col-sm-10 pd-xs-l25 mg-xs-t15">
            <div class="over-hide bg-fff pd-all-15" id="right">
              <!--   <div id="more">
               ...
               <!--  <div id="newsList">
                    <ul>
                        <li><a href="#">我校老年远程教育工作效果显著<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">3号楼装修改造等项招标代理服务招标公告<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">快讯：“珠江•恺撒堡杯”第二届全国老年大学钢琴比赛在广州开锣<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">市老年干部大学举行2016学年结业典礼<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">市委常委、组织部部长王世彤莅临我校调研指导工作<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">全国老年教育会议在广州召开<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">全国老年教育论坛在市老年干部大学举行<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">首届广州地区老年大学协会书画展成功举办<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">国际老年大学协会代表团到广州考察<span style="float: right"> >>> </span></a></li>
                        <li><a href="#">市委老干部局“两学一做”领导小组来我校检查指导学习教育<span style="float: right"> >>> </span></a></li>
                    </ul>
                </div> -->
                 ...
                 <div id="newsList">
                   <%--  <ul>
                       ...
                       <c:forEach items="${detailsList}" var="details" varStatus="status">
                        <li><a href="${lujin}/news?detailsId=${details.detailsId}">${details.title}<span style="float: right"> >>> </span></a></li>
                       </c:forEach>
							...
							<article>
								<header>
									...
									<div class="new_detail_date clearfix">
										发布时间：<fmt:formatDate value='${details.releaseTime}'  pattern="yyyy年M月d日"/>
										
										
									
											　作者：<font color="#003399">${details.author}</font> 
										
									</div>
								</header>
							 	<!--编辑器编辑的内容开始-->
								<!--<p style="color:#333333;font-family:-apple-system-font, BlinkMacSystemFont, &quot;font-size:16px;text-align:justify;background-color:#F4FAEF;">
	<img src="./山东老年大学_files/20190301090649349.png" alt=""> 
</p> --> 
...
<p class="MsoNormal" style="text-align:justify;text-indent:0pt;">
	<span style="font-family:宋体;line-height:200%;font-size:14.0000pt;">${details.content}</span>
</p>

<!-- <p class="MsoNormal" style="text-align:justify;text-indent:28pt;">
	<span style="font-family:宋体;line-height:200%;font-size:14.0000pt;"><span>春回大地，万物复苏。今天，我们迎来了</span>2019年的开学第一天，请允许我代表山东老年大学全体教职员工，向各位学员致以诚挚的问候和春天的祝福！</span><span style="font-family:宋体;line-height:200%;font-size:14.0000pt;"></span> 
</p>

<p class="MsoNormal" style="text-align:justify;">
	<img src="./山东老年大学_files/20190301090028753.webp.jpg" alt=""> 
</p>
 -->
...
<p>
	<br>
								...
								<footer>
									...
									<!-- 	<img width="14" align="absmiddle" height="14" src="./山东老年大学_files/return.png"> -->
   ...
   <!--分页-->
  <!--            <div class="fenye"><div class="manu" style="margin-top:15px"><span class="disabled">< 上一页</span><span class="current">1</span><a href="#" target="_self">2</a><a href="#" target="_self">3</a><a href="#" target="_self">4</a><a href="#" target="_self">5</a>...<a href="#">11</a><a href="#">下一页 ></a></div>
</div> -->
          <!--分页结束-->
