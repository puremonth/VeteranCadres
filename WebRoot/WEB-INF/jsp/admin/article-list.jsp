...
<link rel="stylesheet" type="text/css" href="/VeteranCadres/static/h-ui.admin/skin/default/skin.css" id="skin" />
...
<header class="navbar-wrapper">
	<div class="navbar navbar-fixed-top">
		<div class="container-fluid cl">
			<a class="logo navbar-logo f-l mr-10 hidden-xs" href="/VeteranCadres/admin/article-list">老干部大学后台管理 </a>
			<a class="logo navbar-logo-m f-l mr-10 visible-xs" href="/VeteranCadres/admin/article-list">H-ui</a>
...
<!-- 					<li>超级管理员</li>
					<li class="dropDown dropDown_hover">
						<a href="/VeteranCadres/admin/article-list" class="dropDown_A">admin <i class="Hui-iconfont">&#xe6d5;</i></a>
						<ul class="dropDown-menu menu radius box-shadow">
							<li><a href="javascript:;" onClick="myselfinfo()">个人信息</a></li>
							<li>
								<a href="/VeteranCadres/admin/article-list">切换账户</a>
							</li>
							<li>
								<a href="/VeteranCadres/admin/article-list">退出</a>
							</li>
						</ul>
					</li>
					<li id="Hui-msg">
						<a href="/VeteranCadres/admin/article-list" title="消息">
	...
	<div class="menu_dropdown bk_2">
		<dl id="menu-article">
			...
			<dd style="display:block">
				<ul>
					<li class="current">
						<a href="admin/article-list" title="资讯管理">资讯管理</a>
					</li>
				</ul>
			</dd>
		</dl>
...
<!-- 		<dl id="menu-admin">
			...
			<dd>
				<ul>
					<li>
						<a href="admin/admin-role.html" title="角色管理">角色管理</a>
					</li>
					<li>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
				<a href="javascript:;" onclick="article_del(this)" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
			...
			<div class="mt-20">
			 <form id="objidForm" method="post"  style="display:inline">
     <%-- 	<input id="menufirst" type="hidden" value="${menufirst}"/>
     	<input id="statu" name="statu" type="hidden" value="${statu}"/>
        <input id="menusecond" type="hidden" value="${menusecond}"/> --%>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
					  <%-- <c:forEach items="${detailsList}" var="details" >
                        <li><a href="${lujin}/news?detailsTypeId=${details.detailsTypeId}&detailsId=${details.detailsId}">${details.title}<span style="float: right"> >>> </span></a></li>
                       </c:forEach>  --%>
                       
					<c:forEach items="${details}" var="details" varStatus="status">
						<tr class="text-c">
							<td><input type="checkbox" value="${details.detailsId}" name="id"></td>
							<td id="detailsId">${details.detailsId}</td>
							<td class="text-l"><u style="cursor:pointer" class="text-primary" onClick="article_edit('查看','article-zhang.html',${details.detailsId})" title="查看">${details.title}</u></td>
							<td>
						    <c:if test="${details.detailsTypeId eq '0'}">
							单位概况
							</c:if>
							<c:if test="${details.detailsTypeId eq '1'}">
							党建领航
							</c:if>
							<c:if test="${details.detailsTypeId eq '2'}">
							新闻动态
							</c:if>
							<c:if test="${details.detailsTypeId eq '3'}">
							课程安排
							</c:if>
							<c:if test="${details.detailsTypeId eq '4'}">
							往期课程
							</c:if>
							<c:if test="${details.detailsTypeId eq '5'}">
							专业介绍
							</c:if>
							<c:if test="${details.detailsTypeId eq '6'}">
							教师风采
							</c:if>
							<c:if test="${details.detailsTypeId eq '7'}">
							学员天地
							</c:if>
							<c:if test="${details.detailsTypeId eq '8'}">
							镇街动态
							</c:if>
							<c:if test="${details.detailsTypeId eq '9'}">
							长青杂志
							</c:if>
							<c:if test="${details.detailsTypeId eq '10'}">
							经典回顾
							</c:if>
							<c:if test="${details.detailsTypeId eq '11'}">
							活动剪影
							</c:if>
							<c:if test="${details.detailsTypeId eq '12'}">
							书法作品
							</c:if>
							<c:if test="${details.detailsTypeId eq '13'}">
							绘画作品
							</c:if>
							<c:if test="${details.detailsTypeId eq '14'}">
							瓷绘作品
							</c:if>
							<c:if test="${details.detailsTypeId eq '15'}">
							摄影作品
							</c:if>
							<c:if test="${details.detailsTypeId eq '16'}">
							文学作品
							</c:if>
							<c:if test="${details.detailsTypeId eq '17'}">
						        校报在线
							</c:if>
							</td>
							<td>${details.author}</td>
							<td><fmt:formatDate value='${details.releaseTime}' pattern="yyyy年M月d日"/></td>
							...
							<td class="f-14 td-manage"><!-- <a style="text-decoration:none" onClick="article_stop(this,'10001')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a> -->
								<a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','admin/article-editor','${details.detailsId}','${page.pageNow}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function article_add(title,url,w,h){
    document.getElementById('objidForm').action="admin/article-add";
    document.getElementById('objidForm').submit();
/* 
...
function article_edit(title,url,id,w,h){
    var detailsId=document.getElementById("objid");
  var pageNow=document.getElementById("pageNow");
   // pageNow.value=w;
	detailsId.value=id;
	 document.getElementById('objidForm').action="admin/article-editor";
     document.getElementById('objidForm').submit();
...
function article_del(obj){
	...
	setId();
	  var objid=document.getElementById("objid");
	  document.getElementById('objidForm').action="admin/article-del";
      document.getElementById('objidForm').submit();
	/* 	$.ajax({
			type: 'POST',
